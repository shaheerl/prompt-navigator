...
// Target the master wrapper by looking for any div whose ID ends with "-user-message"
// Alternatively, we look for the specific Tailwind class Copilot uses
const COPILOT_PROMPT_SELECTOR = 'div[id$="-user-message"], .group\\/user-message';

let prompts = [];
let currentIndex = -1;

function collectPrompts() {
  prompts = Array.from(document.querySelectorAll(COPILOT_PROMPT_SELECTOR));
  return prompts;
}
...
function truncateText(text, maxLength = 72) {
  // Keeps our failsafe regex to scrub the visually hidden "You said"
  const cleaned = text
    .trim()
    .replace(/\s+/g, " ")
...
function renderPanel() {
  ...
  prompts.forEach((prompt, index) => {
    ...
    // Copilot images are standard <img> tags
    const hasImage = prompt.querySelector("img");
    const imageIndicator = hasImage ? "📸 " : "";

    // Copilot explicitly tags the text bubble with this data attribute
...
console.log("pn: copilot script loaded");
setTimeout(() => {
  console.log("pn: copilot init fired");
  collectPrompts();
