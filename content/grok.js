...
// We use the "rounded-br-lg" class as a clever hack to only target user message bubbles
const GROK_PROMPT_SELECTOR = '.message-bubble.rounded-br-lg';

let prompts = [];
let currentIndex = -1;

function collectPrompts() {
  prompts = Array.from(document.querySelectorAll(GROK_PROMPT_SELECTOR));
  return prompts;
}
...
console.log("pn: grok script loaded");
setTimeout(() => {
  console.log("pn: grok init fired");
  collectPrompts();
