...
// We use partial class matching to bypass Poe's auto-generated CSS hashes
const POE_PROMPT_SELECTOR = '[class*="ChatMessage_rightSideMessageWrapper"]';

let prompts = [];
let currentIndex = -1;

function collectPrompts() {
  prompts = Array.from(document.querySelectorAll(POE_PROMPT_SELECTOR));
  return prompts;
}
...
function renderPanel() {
  ...
  prompts.forEach((prompt, index) => {
    ...
    // Look for standard image tags inside the prompt wrapper
    const hasImage = prompt.querySelector("img");
    const imageIndicator = hasImage ? "📸 " : "";

    // Target the specific prose container to avoid accidentally scraping the timestamp (e.g., "6:43 PM")
...
console.log("pn: poe script loaded");
setTimeout(() => {
  console.log("pn: poe init fired");
  collectPrompts();
