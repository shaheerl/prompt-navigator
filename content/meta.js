// prompt navigator - meta ai

// Meta makes it incredibly easy with a dedicated data attribute
const META_PROMPT_SELECTOR = '[data-message-type="user"]';

let prompts = [];
let currentIndex = -1;

function collectPrompts() {
  prompts = Array.from(document.querySelectorAll(META_PROMPT_SELECTOR));
  return prompts;
}
...
function renderPanel() {
  ...
  prompts.forEach((prompt, index) => {
    ...
    // Target the specific text-response span to avoid grabbing timestamps/button labels
...
console.log("pn: meta script loaded");
setTimeout(() => {
  console.log("pn: meta init fired");
  collectPrompts();
